...
-(void)requestDataPrivate:(NSArray*)parmArr
{
    ...
    else
    {
        ...
        for (NSString *key in keyArr) {
            id value = [networkEntity.NRequestParamDir objectForKey:key];
            if ([value isKindOfClass:[NSMutableDictionary class]]) {
                NSMutableDictionary* dic = value;
//                [request addData:[dic objectForKey:KPOSTDATA] withFileName:[dic objectForKey:KPOSTFILENAME] andContentType:[dic objectForKey:KPOSTCONTENTTYPE] forKey:[dic objectForKey:KPOSTKEY]];
                NSString *fileName = [[NSBundle mainBundle] pathForResource:[dic objectForKey:KPOSTFILENAME] ofType:[dic objectForKey:KPOSTCONTENTTYPE]]; 
                [request setFile: fileName forKey: key];  
            }
            else
