...
+ (GTMABAddressBook *)addressBook:(ABAddressBookRef)addressBookRef {
    return [[[self alloc] initWith:addressBookRef] autorelease];
}

- (id)initWith:(ABAddressBookRef)addressBookRef {
    if ((self = [super init])) {
        addressBook_ = addressBookRef;
        addressBookFromOut = YES; 
        if (!addressBook_) {
            // COV_NF_START
            [self release];
            self = nil;
...
- (void)dealloc {
  if (addressBook_&&!addressBookFromOut) {
