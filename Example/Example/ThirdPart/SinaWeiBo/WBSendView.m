...
- (id)initWithAppKey:(NSString *)appKey appSecret:(NSString *)appSecret text:(NSString *)text image:(UIImage *)image
{
    if (self = [super initWithFrame:CGRectMake(0, 0, 320, 480)])
    {
        ...
        contentTextView = [[UITextView alloc] initWithFrame:CGRectMake(13, 60, 288 - 26, 150-24)];
        ...
        //账号绑定
        bindAcountButton = [UIButton buttonWithType:UIButtonTypeCustom];
		[bindAcountButton setShowsTouchWhenHighlighted:YES];
		[bindAcountButton setFrame:CGRectMake(15, 198, 60, 30)];
        [bindAcountButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
		[bindAcountButton setBackgroundImage:[UIImage imageNamed:@"btn2.png"] forState:UIControlStateNormal];
        if ([engine isLoggedIn] && ![engine isAuthorizeExpired])
        {

            [bindAcountButton setTitle:NSLocalizedString(@"已绑定", nil) forState:UIControlStateNormal];
            bindAcountButton.enabled = FALSE;
        }
        else
        {
            [bindAcountButton setTitle:NSLocalizedString(@"未绑定", nil) forState:UIControlStateNormal];
            bindAcountButton.enabled = YES;

        }
		[bindAcountButton.titleLabel setFont:[UIFont boldSystemFontOfSize:12.0f]];
		[bindAcountButton addTarget:self action:@selector(onBindAccountButtonTouched:) forControlEvents:UIControlEventTouchUpInside];
		[panelView addSubview:bindAcountButton];
        
        
        //切换账号
        changeAcountButton = [UIButton buttonWithType:UIButtonTypeCustom];
		[changeAcountButton setShowsTouchWhenHighlighted:YES];
		[changeAcountButton setFrame:CGRectMake(15+75, 198, 60, 30)];
        [changeAcountButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
		[changeAcountButton setBackgroundImage:[UIImage imageNamed:@"btn2.png"] forState:UIControlStateNormal];
        if ([engine isLoggedIn] && ![engine isAuthorizeExpired])
        {

            changeAcountButton.hidden = NO;
            UIImageView  *imageview = [[UIImageView alloc]initWithFrame:CGRectMake(22, 5, 19, 20)];
            imageview.image = [UIImage imageNamed:@"qiehuan.png"];
            [changeAcountButton addSubview:imageview];
            [imageview release];
        }
        else
        {

             changeAcountButton.hidden = YES;
        }
        
		[changeAcountButton.titleLabel setFont:[UIFont boldSystemFontOfSize:12.0f]];
		[changeAcountButton addTarget:self action:@selector(onChangeAccountButtonTouched:) forControlEvents:UIControlEventTouchUpInside];
		[panelView addSubview:changeAcountButton];
        ...
        if (image)
        {
			...
			if (width > height) {
				tframe.size.width = 80;
				tframe.size.height = height * (80 / width);
			}
			else {
			...
			[contentImageView setCenter:CGPointMake(144, 280)];
...
- (void)onBindAccountButtonTouched:(id)sender
{
  [contentTextView resignFirstResponder];
  [self performSelector:@selector(toAuthorizeWeb:) withObject:nil afterDelay:0.4];
}
- (void)toAuthorizeWeb:(id)sender
{
    isToAuthorize = YES;
    [engine logIn];
}
- (void)onChangeAccountButtonTouched:(id)sender
{
    [contentTextView resignFirstResponder];
    [engine logOut];
    [self performSelector:@selector(toAuthorizeWeb:) withObject:nil afterDelay:0.4];
...
- (void)sizeToFitOrientation:(UIInterfaceOrientation)orientation
{
    ...
    else
    {
        ...
        [contentTextView setFrame:CGRectMake(13, 60, 288 - 26, 150-24)];
        [contentImageView setCenter:CGPointMake(144, 280)];
        [clearImageButton setCenter:CGPointMake(contentImageView.center.x + contentImageView.frame.size.width / 2,
                                                contentImageView.center.y - contentImageView.frame.size.height / 2)];
...
//    }
    [DAUtility hideHUD];
//    [indicatorView stopAnimating];
    [self onBindAccountButtonTouched:self];
}
...
//绑定登陆成功
- (void)engineDidLogIn:(WBEngine *)engine1
{
    
    if ([engine1 isLoggedIn] && ![engine1 isAuthorizeExpired])
    {
...
- (void)engine:(WBEngine *)engine1 didFailToLogInWithError:(NSError *)error
{
    if ([engine1 isLoggedIn] && ![engine1 isAuthorizeExpired])
    {
