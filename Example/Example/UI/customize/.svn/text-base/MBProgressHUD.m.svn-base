...
static MBProgressHUD *s_hud  = nil;
static MBProgressHUD  *s_hud_noCover = nil;
+ (MBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated coverType:(MBProgressCover_Type) type
{
    if (PROGRESS_HUD_TYPE_COVER == type)
    {
        if (nil == s_hud) 
        {
            s_hud = [[MBProgressHUD alloc] initWithView:view];
            s_hud.m_type = PROGRESS_HUD_TYPE_COVER;
        }
        [view addSubview:s_hud];
        [s_hud show:animated];
        return s_hud;

    }
    else if(PROGRESS_HUD_TYPE_NOCOVER == type)
    {
        if (nil == s_hud_noCover) 
        {
            
            s_hud_noCover = [[MBProgressHUD alloc] initWithViewNocover:view];
            s_hud_noCover.m_type = PROGRESS_HUD_TYPE_NOCOVER;
        }
        [view addSubview:s_hud_noCover];
        [s_hud_noCover show:animated];
        return s_hud_noCover;
    }
    return nil;
    //helei 2011-9-23
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated 
{
    ...
    NSMutableArray *hudviewArray = [NSMutableArray arrayWithCapacity:1];
	for (UIView *v in [view subviews])
    {
		if ([v isKindOfClass:[MBProgressHUD class]]) 
        {
			viewToRemove = v;
            [hudviewArray addObject:v];
		}
	}
	
    if ([hudviewArray count] >0)
    {
        for (int i = 0; i<[hudviewArray count]; i++)
        {
            MBProgressHUD *HUD = (MBProgressHUD *)[hudviewArray objectAtIndex:i];
...
- (id)initWithViewNocover:(UIView *)view 
{
    ...
    id me =  [self initWithFrame:CGRectMake(127, 151, 77, 77)];
    ...
    //    int currentIndex = [self.superview.subviews indexOfObject:self]; 
    //    printLog(@"%d",currentIndex);
    
    //helei:2011.10.31先不支持横书评切换，崩溃问题的可能
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        self.opacity = 0.8;//tank:2011.8.13不透明度
...
/*
 processOpacity 0.0~1.0之间
 */
- (id)initWithFrame:(CGRect)frame 
processOpacity:(float)processOpacity
{
    if ((self = [super initWithFrame:frame])) {
        // Set default values for properties
        self.animationType = MBProgressHUDAnimationFade;
        ...
        self.labelText = nil;
        self.detailsLabelText = nil;
        self.opacity = processOpacity;//tank:2011.8.13不透明度
        self.labelFont = [UIFont boldSystemFontOfSize:LABELFONTSIZE];
        self.detailsLabelFont = [UIFont boldSystemFontOfSize:LABELDETAILSFONTSIZE];
        self.xOffset = 0.0;
        self.yOffset = 0.0;
...
- (void)layoutSubviews 
{
    ...
    if (PROGRESS_HUD_TYPE_NOCOVER == self.m_type) 
    {
        indFrame.origin.x = (self.width - indFrame.size.width)/2;
        indFrame.origin.y = 10;
    }
    else if(PROGRESS_HUD_TYPE_COVER == self.m_type)
    {
    ...
    if (nil != self.labelText) 
    {
        ...
        float lWidth;
        if (PROGRESS_HUD_TYPE_COVER == self.m_type) 
        {
        ...
        if (PROGRESS_HUD_TYPE_NOCOVER == self.m_type) 
        {
            ...
            indicator.frame = indFrame;
            [self setFrame:CGRectMake((320-self.width)/2, (460-self.height)/2+self.yOffset, self.width, self.height)];
            lFrame = CGRectMake(0, floor(indFrame.origin.y + indFrame.size.height + PADDING),self.width, lHeight);
            //end helei;
        }
        else if(PROGRESS_HUD_TYPE_COVER == self.m_type)
        {
        ...
        if (nil != self.detailsLabelText) {
            ...
            detailsLabel.shadowColor = [UIColor colorWithWhite:0.0f alpha:0.5f];
            detailsLabel.shadowOffset = CGSizeMake(0, 1);
...
- (void)drawRect:(CGRect)rect {
    ...
    if (PROGRESS_HUD_TYPE_NOCOVER == self.m_type) 
    {
        boxRect = CGRectMake(0, 0, self.width, self.height);
    }
    else if(PROGRESS_HUD_TYPE_COVER == self.m_type)
    {
        boxRect = CGRectMake(((allRect.size.width - self.width) / 2) + self.xOffset,
...
- (void)deviceOrientationDidChange:(NSNotification *)notification { 
    ...
    //tank:2011.7.11先不支持横书评切换，崩溃问题的可能
#if 0
...
#else
    return [super initWithFrame:CGRectMake(0.0f, 0.0f, 44.0f, 44.0f)];
#endif
}
...
- (void)drawRect:(CGRect)rect {
    ...
    // Draw background
//    CGContextSetRGBStrokeColor(context, 1.0, 1.0, 0.0, 0.0); // white
//    CGContextSetRGBFillColor(context, 1.0, 1.0, 1.0, 1.0); // translucent white
...
//    CGContextStrokeEllipseInRect(context, circleRect);
    ...
    CGContextAddArc(context, x, y, (allRect.size.width - 4) / 2, -(PI / 2), (1 * 2 * PI) - PI / 2, 0);
    CGContextClosePath(context);
    CGContextFillPath(context);
    
    
    
    CGContextSetRGBFillColor(context, 162.0/255.0, 208.0/255.0, 55.0/255.0, 1.0); // white
    CGContextMoveToPoint(context, x, y);
