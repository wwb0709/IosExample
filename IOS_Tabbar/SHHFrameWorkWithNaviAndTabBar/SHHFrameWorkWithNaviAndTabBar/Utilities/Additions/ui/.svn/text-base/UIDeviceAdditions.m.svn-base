...
static NSString *hardwareInfoAsString(const char *keyPath) {
	...
	if (sysctlbyname(keyPath, buffer, &size, NULL, 0) == 0) {
		NSData *bufferData = [[NSData alloc] initWithBytes:buffer length:(size - 1)]; // Trim off /Users/appleapple/Mtv/MtvClient/MtvClient/Utils/Additions/ui/UIDeviceAdditions.mthe last character which is \0.
